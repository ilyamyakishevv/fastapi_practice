...
def start_message(message):
    try: 
        user = pydantic_models.UserToCreate(tg_ID=message.from_user.id, nick=message.from_user.username)
...
def wallet(message): 
    ...
    markup.add(btn1)
    text = f"Баланс вашего колешька: {wallet['balance']} BTC\n Адрес вашего кошелька: {wallet['address']}"
    bot.send_message(message.chat.id, text, reply_markup=markup)
